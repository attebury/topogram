...
status: canonical
summary: Current user journey map for Topogram packages, templates, generators, importer packs, extractors, catalog entries, and agent workflows.
...
affects:
  ...
  - journey_importer_author_publishes_pack
...
| `journey_brownfield_import_and_adoption` | Maintainer, agent | Importer packs, extractors, candidate output | Import emits reviewable candidates; adoption writes canonical `topo/**` only when explicit. |
...
| `journey_importer_author_publishes_pack` | Importer author | Importer package, extractors | Importer packs execute discovery only; core owns persistence, reconcile, and adoption. |
...
- Execution packages are installed, policy-checked, and invoked: generators and importer packs.
- Extractors are units inside importer packs, not user-facing content packages.
...
Package-backed importers need their own manifest and policy surface before we add external importer packs. The journey map says importer packs should mirror generators in package handling, but their output contract must be candidate/evidence-only.
