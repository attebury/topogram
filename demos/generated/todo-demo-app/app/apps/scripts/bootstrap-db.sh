...
(cd "$ROOT_DIR/db/todo_postgres" && TOPOGRAM_ENV_FILE=/dev/null DATABASE_URL="${DATABASE_URL:-}" DATABASE_ADMIN_URL="${DATABASE_ADMIN_URL:-}" TOPOGRAM_RUNTIME_SERVER_DIR="$ROOT_DIR/services/todo_api" bash ./scripts/db-bootstrap-or-migrate.sh)
...
(cd "$ROOT_DIR/services/todo_api" && npm install && npm exec -- prisma generate --schema prisma/schema.prisma && npm exec -- prisma db push --schema prisma/schema.prisma --skip-generate && npm run seed:demo)
fi
