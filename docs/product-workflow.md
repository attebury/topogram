...
2. edit `topo/` and `topogram.project.json`
3. run `topogram check`
4. run `topogram generate`
5. compile or smoke-check the generated app
6. repeat

`topogram.project.json` owns stack and topology choices. It binds generated or maintained outputs, API/web/database widgets, ports, and generator IDs. Topogram core validates that topology and builds normalized contracts; generator packs own the stack-specific realization. Generated outputs can be regenerated when `.topogram-generated.json` is present; maintained outputs are not overwritten.
