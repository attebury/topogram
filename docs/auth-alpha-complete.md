...
- **Primary signed-token profile**
  ...
  - Status: complete for alpha. The profile verifies HS256 JWT signatures, extracts claims, rejects missing, malformed, invalid-signature, and expired bearer tokens, and fails closed when a protected route has no configured auth profile.
...
- **Generated proof loop**
  ...
  - Status: complete for alpha. The issues runtime-check plan includes `401` missing-token, invalid-signature, and expired-token cases plus `403` ownership denial. The content-approval runtime-check plan includes `403` for a valid token without the required reviewer claim.
