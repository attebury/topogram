...
function envValue(name) {
  ...
  const fallbackMap = {
    TOPOGRAM_API_BASE_URL: process.env.PUBLIC_TOPOGRAM_API_BASE_URL || "http://localhost:3002",
    TOPOGRAM_WEB_BASE_URL: process.env.PUBLIC_TOPOGRAM_WEB_BASE_URL || `http://localhost:${process.env.WEB_PORT || "5175"}`,
...
async function runCheck(definition) {
  ...
  try {
    ...
    } else if (definition.id === "web_article_detail_reviewer_actions_visible" || definition.id === "web_article_detail_reviewer_actions_hidden") {
      const bodyText = await runSafariBrowserCheck(definition);
      if (definition.expectText) {
