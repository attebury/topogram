...
INPUT_PATH="${TOPOGRAM_INPUT_PATH:-$REPO_ROOT/examples/todo/topogram}"
...
refresh_runtime_clients() {
  ...
  if [[ -f "$BUNDLE_DIR/../server/package.json" ]]; then
    (cd "$BUNDLE_DIR/../server" && npm install && npm exec -- prisma generate --schema "$PRISMA_SCHEMA" && npm exec -- prisma db push --schema "$PRISMA_SCHEMA")
    return
  fi
  npx -p "prisma@$prisma_version" prisma generate --schema "$PRISMA_SCHEMA"
  npx -p "prisma@$prisma_version" prisma db push --schema "$PRISMA_SCHEMA"
}
