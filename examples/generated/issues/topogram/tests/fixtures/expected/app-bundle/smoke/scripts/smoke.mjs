...
const getResponse = await fetch(new URL(`/issues/${created.id}`, apiBase), {
  headers: authToken ? { Authorization: `Bearer ${authToken}` } : undefined
});
await expectStatus(getResponse, 200, "get resource");

const listResponse = await fetch(new URL("/issues", apiBase), {
  headers: authToken ? { Authorization: `Bearer ${authToken}` } : undefined
});
